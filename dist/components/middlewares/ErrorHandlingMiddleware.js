...
let ErrorHandlingMiddleware = ErrorHandlingMiddleware_1 = class ErrorHandlingMiddleware {
    static setLogger(logger) {
        ErrorHandlingMiddleware_1.logger = logger;
    }
    error(error, {}, response, next) {
        const extractedError = this.extractError(error);
        this.logError(extractedError);
        const coreHttpError = extractedError instanceof http_errors_1.HttpError ? extractedError : this.createCoreHttpError(extractedError);
        let code;
        let data;
        if (coreHttpError) {
